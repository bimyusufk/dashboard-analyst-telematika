...
// [USER_ACTION]: Ganti URL ini dengan path ke file CSV di folder 'public' proyek Anda.
// Contoh: "/data/hasil_survey.csv"
...
export default function ResearchDashboard() {
  ...
  // --- CSV PARSING LOGIC ---
  // Fungsi ini mereplikasi logika Python untuk mengubah data mentah CSV menjadi skor variabel
  const parseCSVData = (csvText) => {
    const lines = csvText.trim().split('\n');
    const headers = lines[0].split(','); // Baris pertama adalah header
    
    // Mulai dari baris 1 (karena baris 0 adalah header)
    const parsedData = lines.slice(1).map((line, index) => {
      // Regex ini untuk menangani koma di dalam tanda kutip (jika ada)
      // Namun untuk data survey Likert sederhana, split(',') biasanya cukup
      const values = line.split(/,(?=(?:(?:[^"]*"){2})*[^"]*$)/);
      
      // Helper untuk ambil nilai integer dari kolom tertentu (0-based index)
      const getVal = (idx) => parseInt(values[idx]) || 0;

      // --- MAPPING KOLOM (SESUAI ANALISIS PYTHON) ---
      // Intensity: Kolom [2, 4, 5, 18, 19]
      const intensityScore = (getVal(2) + getVal(4) + getVal(5) + getVal(18) + getVal(19)) / 5;
      
      // Dependency: Kolom [3, 6, 20, 21, 22]
      const dependencyScore = (getVal(3) + getVal(6) + getVal(20) + getVal(21) + getVal(22)) / 5;
      
      // Competence F2F: Kolom [7, 8, 9, 10]
      const competenceScore = (getVal(7) + getVal(8) + getVal(9) + getVal(10)) / 4;
      
      // Alienation: Kolom [13, 14, 15, 16, 17]
      const alienationScore = (getVal(13) + getVal(14) + getVal(15) + getVal(16) + getVal(17)) / 5;
      
      // Boldness: Kolom 11
      const boldnessScore = getVal(11);

      // Usage Group Logic (Kolom 2: Durasi > 6 Jam)
      // Skala 1-5, asumsikan 4 (Setuju) dan 5 (Sangat Setuju) adalah > 6 Jam
    ...
    return parsedData.filter(d => !isNaN(d.intensity)); // Filter baris kosong/error
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.warn("Menggunakan data fallback karena:", error.message);
  ...
  const avgAlienation = (data.reduce((a, b) => a + b.alienation, 0) / data.length).toFixed(2);
  const correlationVal = 0.51; // Hardcoded hasil Spearman (Python)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              ...
              {usingFallback ? <span className="text-amber-600 font-bold ml-1">(Mode: Simulasi / Fallback Data)</span> : <span className="text-emerald-600 font-bold ml-1">(Mode: Live CSV Data)</span>}
            </p>
          </div>
        </div>
      ...
      {/* Alert jika menggunakan fallback */}
      {usingFallback && (
        <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 text-xs text-amber-800 flex justify-center">
          <span>⚠️ <strong>Perhatian:</strong> Dashboard menampilkan data cadangan. Untuk menggunakan data lokal, pastikan file CSV ada di <code>public{CSV_DATA_URL}</code> dan server lokal berjalan.</span>
        </div>
      )}
                 ...
                 <div className="bg-gradient-to-br from-slate-800 to-slate-900 text-white p-6 rounded-xl shadow-md">
                    ...
                    <ul className="space-y-3 text-sm text-slate-300">
                      <li className="flex gap-2">
                        ...
                        <span>Korelasi positif kuat antara <strong>Ketergantungan</strong> dan <strong>Alienasi</strong> ($\rho=0.51$).</span>
                      </li>
                      <li className="flex gap-2">
                        <ChevronRight size={16} className="mt-1 text-blue-400 shrink-0" />
